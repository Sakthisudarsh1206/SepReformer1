...
class Engine(object):
    def __init__(self, args, config, model, dataloaders, criterions, optimizers, schedulers, gpuid, device):
        ...
        self.checkpoint_path = self.pretrain_weights_path if any(file.endswith(('.pt', '.pkl')) for file in os.listdir(self.pretrain_weights_path)) else self.scratch_weights_path
    ...
    def _train(self, dataloader, epoch):
        ...
        for input_sizes, mixture, src, _ in dataloader:
            ...
            cur_loss_s_bn = [self.PIT_SISNR_mag_loss(estims=estim_src_value, idx=idx, input_sizes=input_sizes, target_attr=src) for idx, estim_src_value in enumerate(estim_src_bn)]
            for idx, loss in enumerate(cur_loss_s_bn):
                tot_loss_freq[idx] += loss.item() / self.config['model']['num_spks']
            ...
            cur_loss /= self.config['model']['num_spks']
            cur_loss.backward()
    ...
    def _test(self, dataloader, wav_dir=None):
        ...
        pbar = tqdm(total=len(dataloader), unit='batches', colour="grey", dynamic_ncols=True)
        with torch.inference_mode():
            ...
            with open(csv_file_name_sisnr, 'w', newline='') as csvfile_sisnr, open(csv_file_name_sdr, 'w', newline='') as csvfile_sdr:
                ...
                for input_sizes, mixture, src, key in dataloader:
                    if len(key) > 1: raise("batch size is not one!!")
                    ...
                    if self.engine_mode == "test_save":
                        if wav_dir is None: wav_dir = os.path.join(os.path.dirname(__file__),"wav_out")
                        os.makedirs(wav_dir, exist_ok=True)
                        mixture_np = torch.squeeze(mixture).cpu().data.numpy()
                        sf.write(os.path.join(wav_dir, key[0][:-4]+str(idx)+'_mixture.wav'), 0.5*mixture_np/max(abs(mixture_np)), 8000)
    ...
    def _inference_sample(self, sample):
        ...
        padding = (stride - mixture.shape[-1] % stride) % stride
        ...
        with torch.inference_mode():
            ...
            sf.write(sample[:-4]+'_in.wav', 0.9*mixture_np/max(abs(mixture_np)), self.fs)
            for i in range(self.config['model']['num_spks']):
        ...
        with torch.cuda.device(self.device):
            ...
            if "test" in self.engine_mode:
                ...
                if self.start_epoch > 1:
                    init_loss_time, init_loss_freq, _ = self._validate(self.dataloaders['valid'])
                else:
                ...
                for epoch in range(self.start_epoch, self.config['engine']['max_epoch']):
                    ...
                    logger.info(f"[TRAIN] Epoch {epoch:2d}: Loss_t = {train_loss_src_time:.4f} | Loss_f = {train_loss_src_freq:.4f} | Speed = ({train_end_time - train_start_time:.2f}s/{train_num_batch:d})")
                    logger.info(f"[VALID] Epoch {epoch:2d}: Loss_t = {valid_loss_src_time:.4f} | Loss_f = {valid_loss_src_freq:.4f} | Speed = ({valid_end_time - valid_start_time:.2f}s/{valid_num_batch:d})")
                    if epoch in self.config['engine']['test_epochs']:
                        ...
                        on_test_end = time.time()
                        logger.info(f"[TEST] Epoch {epoch:2d}: SISNRi = {test_loss_src_time_1:.4f} | SDRi = {test_loss_src_time_2:.4f} | Speed = ({on_test_end - on_test_start:.2f}s/{test_num_batch:d})")
                    valid_loss_best = util_engine.save_checkpoint_per_best(
                        valid_loss_best, valid_loss_src_time, train_loss_src_time,
                    ...
                    writer_src.add_scalars("Metrics", {
                        ...
                        'Loss_valid_time': valid_loss_src_time
                    }, epoch)
