...
def save_checkpoint_per_best(best, valid_loss, train_loss, epoch, model, optimizer, checkpoint_path, wandb_run=None):
    ...
    Args:
        best (float): Best validation loss so far.
        ...
        optimizer (Optimizer): The optimizer whose state needs to be saved.
        checkpoint_path (str): Directory path where the checkpoint will be saved.
        wandb_run (wandb.wandb_run.Run, optional): The current wandb run to log and save the checkpoint.
    ...
    if valid_loss < best:
        ...
        # If a wandb run is provided, log and save the checkpoint file
...
def model_params_mac_summary(model, input, dummy_input, metrics):
    ...
    if 'ptflops' in metrics:
        ...
        MACs_ptflops, params_ptflops = (
            MACs_ptflops.replace(" MMac", ""),
            params_ptflops.replace(" M", "")
        )
    ...
    if 'torchinfo' in metrics:
        ...
        MACs_torchinfo, params_torchinfo = (
            model_profile.total_mult_adds/1e6,
            model_profile.total_params/1e6
        )
